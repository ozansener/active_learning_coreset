...
def train(hold_out, dev_name, sample, batch_id,go_on):
    ...
    with tf.Session(config=config) as sesh:
        robust_cifar_train = AdverserialTrainer(params.learning_rate, params.learning_rate, dev_name,10)
        # no need to initialize anything since ve are simply re-loading the data
        saver = tf.train.Saver(max_to_keep=100)
        saver.restore(sesh, 'models/cifar10_go_on_{}_robust_{}_model_hold_out_{}__{}-{}'.format(True,sample,hold_out_s,str_v, batch_id))
        ...
        f,l,lgt = robust_cifar_train.active_sample(sesh,
                                                  {'images':train_data.hold_out.images,
                                                   'labels':train_data.hold_out.labels}, 5000, 10)
        f_t,l_t,lgt_t = robust_cifar_train.active_sample(sesh,
                                                  {'images':train_data.train.images,
                                                   'labels':train_data.train.labels}, 5000, 10)
        #ch = RobustTrainer.samp(1 - gt[:-5000], 5000, 0.25)
        #pickle.dump({'f':f,'l':v,'gt_f':f_t,'gt_l':v_t}, open('chosen_data_{}_{}'.format(hold_out, sample), 'wb'))
