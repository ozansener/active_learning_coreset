...
def extract_test_data(work_directory, one_hot):
    ...
    data_files = glob.glob(work_directory+"/test")
    ...
    for i,fil in enumerate(data_files):
        with open(fil, 'rb') as f:
            ...
            labels[i*10000:(i+1)*10000] = dat_dict['fine_labels']
...
def extract_train_data(work_directory, one_hot, hold_out_size):
    ...
    data_files = glob.glob(work_directory+"/train")
    num_images = len(data_files)*50000;
        ...
        with open(fil, 'rb') as f:
            ...
            if True:
                if True:
                    ...
                    hold_out_size_r = hold_out_size - i*50000
                    if hold_out_size_r > 0:
                        hold_out_images[i*50000:i*50000+hold_out_size_r,:,:,0] = ser_dat[0:hold_out_size_r,0:1024].reshape((-1,32,32)).astype('float32') - 125.0
                        hold_out_images[i*50000:i*50000+hold_out_size_r,:,:,1] = ser_dat[0:hold_out_size_r,1024:2048].reshape((-1,32,32)).astype('float32') - 123.0
                        hold_out_images[i*50000:i*50000+hold_out_size_r,:,:,2] = ser_dat[0:hold_out_size_r,2048:3072].reshape((-1,32,32)).astype('float32') - 114.0
                        hold_out_labels[i*50000:i*50000+hold_out_size_r] = dat_dict['fine_labels'][0:hold_out_size_r]

                    remaining_size = 50000 - hold_out_size_r
                    images[0:remaining_size,:,:,0] = ser_dat[hold_out_size_r:50000,0:1024].reshape((-1,32,32)).astype('float32') - 125.0
                    images[0:remaining_size,:,:,1] = ser_dat[hold_out_size_r:50000,1024:2048].reshape((-1,32,32)).astype('float32') - 123.0
                    images[0:remaining_size,:,:,2] = ser_dat[hold_out_size_r:50000,2048:3072].reshape((-1,32,32)).astype('float32') - 114.0
                    labels[0:remaining_size] = dat_dict['fine_labels'][hold_out_size_r:50000]
...
def read_data_sets(train_dir,
    ...
    DATA_SET_NAME = "cifar-100-python.tar.gz"
